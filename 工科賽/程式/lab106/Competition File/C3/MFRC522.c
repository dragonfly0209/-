...
unsigned char SPI_transfer(unsigned char data) {
    ...
    for (cnt = 0; cnt < 8; cnt++) {
        SCK = 0;
		Delay10TCYx(24);
        MOSI = (data & 0x80) ? 0b1 : 0b0;
        data = data << 1;
        SCK = 1;
		Delay10TCYx(24);
        readData = (readData << 1) + MISO;
    }
    return readData;
}
...
void PCD_WriteRegister(PCD_Register reg, byte value) {
    // Activate MFRC522.
    MFRC522_CS = 0;
    // MSB must be 0 for writing.
    SPI_transfer(0x7F & reg);
    SPI_transfer(value);
    // Deactivate MFRC522.
    MFRC522_CS = 1;
}
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_WriteRegisters(PCD_Register reg, byte count, byte *values) {
    unsigned char index;
    // Activate MFRC522.
    MFRC522_CS = 0;
    // MSB must be 0 for writing.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void PCD_ReadRegisters(PCD_Register reg, byte count, byte *values, byte rxAlign) {
    if (count != 0) {
        ...
        for (index = 0; index < count - 1; index++) {
            values[index] = SPI_transfer(0x80 | reg);
        }
        // Send 0 to stop reading.
        values[index] = SPI_transfer(0x00);
        // Deactivate MFRC522.
        MFRC522_CS = 1;
    }
}
 ...
 */
void PCD_Init() {
    // Hold RST low for hard reset.
    MFRC522_RST = 0;
    // Delay for safety.
    Delay1KTCYx(240);       //240k * 4 / 48M = 20ms
    Delay1KTCYx(240);       //240k * 4 / 48M = 20ms
    Delay1KTCYx(120);       //120k * 4 / 48M = 10ms
    // Release RST.
    MFRC522_RST = 1;
...
StatusCode MIFARE_Write(	byte blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
	...
	if (result != STATUS_OK) {
        printf("Error: Tell the PICC we want to write to block blockAddr.\r\n");
		return result;
	}
	...
	if (result != STATUS_OK) {
        printf("Error: Transfer the data.\r\n");
		return result;
	}
...
void PICC_DumpMifareClassicToSerial(	Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	if (no_of_sectors) {
        printf("Sector Block 00 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 AccessBits\r\n");
...
void PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	for (blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
		...
		if (firstInGroup) {
			// Print access bits
			printf(" [ ");
			printf("%01d", (g[group] >> 2) & 1); printf(" ");
			printf("%01d", (g[group] >> 1) & 1); printf(" ");
			printf("%01d", (g[group] >> 0) & 1);
			printf(" ] ");
			if (invertedError) {
